class BubbleLetter{
  ...
  BubbleLetter(ParticleSystem w, Vector3D center){
    ...
    for( int i=0; i<bubbles.length; i++ ){
      ...
      system.addParticle( bubbles[i] );
      anchors[i] = system.makeParticle(0, 1, center.x(), center.y(), 0);
      anchors[i].makeInvisible();
      anchors[i].makeFixed();
      connections[i] = new Spring(bubbles[i], anchors[i], 3, 1, 0);
      system.addForce(connections[i]);
      drags[i] = new Drag(bubbles[i], 0.96);
      system.addForce(drags[i]);
  ...
  void detachLetter(){
    ...
    for( int i=0; i<bubbles.length; i++ ){
      ...
      system.addForce(new GravityPoint(bubbles[i], -0.2, center ));
  ...
  void addSurrounding( float x, float y ){
    colorMode(HSB,1);
    Particle p = system.addParticle( new ColoredParticle( surroundingColor() , random(5,25), 0.5, new Vector3D(x, y, 0) ) );
    surrounding.add( p );
    system.addForce(new GravityPoint(p, 0.25, center ));
  }
  ...
  void removeSurrounding(){
    ...
    while( surroundingIter.hasNext() ){
      ColoredParticle bubble = (ColoredParticle) surroundingIter.next();
      system.addForce( new GravityPoint( bubble, -2, center ) );
    }
