class BubbleLetter{
  ...
  BubbleLetter( ParticleSystem w, Vector3D c, Letter l ){
    this(w,c);
    make(l);
  }
  
  BubbleLetter(ParticleSystem w, Vector3D c){
    this.world = w;
    ...
    for( int i=0; i<bubbles.length; i++ ){
      bubbles[i] = new ColoredParticle( letterColor(), 0, 10, new Vector3D(random(-20,20)+center.x, random(-20,20)+center.y, 0) );
      world.addParticle( bubbles[i] );
      anchors[i] = world.makeParticle(0, 1, 0,0,0);
  ...
  void detachLetter(){
    ...
    for( int i=0; i<bubbles.length; i++ ){
      ...
      world.addForce(new GravityPoint(bubbles[i], 0.25, center ));
